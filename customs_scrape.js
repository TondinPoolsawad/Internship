...
async function uploadCsvToSupabase(localPath, bucketFilename) {
  ...
  const client = createClient("https://mzzyjtlrbbwqdxpenwne.supabase.co", key);
  ...
  console.log(`☁️  Uploaded → csvs/${bucketFilename}`);
}
...
async function main() {
    ...
    for (const hs of hsCodes) {
      ...
      for (const { year, month } of monthRange(startYM, endYM)) {
        ...
        if (q) {
          ...
          console.log(`  ${year}-${String(month).padStart(2, "0")}: 0`);
  ...
  await uploadCsvToSupabase(output, "customs.csv");
}
