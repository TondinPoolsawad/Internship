...
const MONTHS_TH = ["α╕íα╕üα╕úα╕▓α╕äα╕í","α╕üα╕╕α╕íα╕áα╕▓α╕₧α╕▒α╕Öα╕ÿα╣î","α╕íα╕╡α╕Öα╕▓α╕äα╕í","α╣Çα╕íα╕⌐α╕▓α╕óα╕Ö","α╕₧α╕ñα╕⌐α╕áα╕▓α╕äα╕í","α╕íα╕┤α╕ûα╕╕α╕Öα╕▓α╕óα╕Ö","α╕üα╕úα╕üα╕Äα╕▓α╕äα╕í","α╕¬α╕┤α╕çα╕½α╕▓α╕äα╕í","α╕üα╕▒α╕Öα╕óα╕▓α╕óα╕Ö","α╕òα╕╕α╕Ñα╕▓α╕äα╕í","α╕₧α╕ñα╕¿α╕êα╕┤α╕üα╕▓α╕óα╕Ö","α╕ÿα╕▒α╕Öα╕ºα╕▓α╕äα╕í"];
const MONTH_WORDS = [...MONTHS_EN, ...MONTHS_TH];
const MONTH_RE = new RegExp(MONTH_WORDS.join("|"), "i");
const ANNUAL_SPAN_RE = /(january[^a-z]*december|jan[^a-z]*dec|α╕íα╕üα╕úα╕▓α╕äα╕í[^α╕ü-α╣Ö]*α╕ÿα╕▒α╕Öα╕ºα╕▓α╕äα╕í)/i;
const HALF_SPAN_RE   = /(january[^a-z]*june|jan[^a-z]*jun|α╕äα╕úα╕╢α╣êα╕çα╕¢α╕╡|α╕íα╕üα╕úα╕▓α╕äα╕í[^α╕ü-α╣Ö]*α╕íα╕┤α╕ûα╕╕α╕Öα╕▓α╕óα╕Ö)/i;
...
function smartYearFrom(text = "", href = "", neighborText = "") {
  ...
  for (const src of sources) {
    ...
    while ((m = re.exec(src))) {
      ...
      if (y >= 2500) y -= 543; // Thai ΓåÆ Gregorian
      entries.push({ y, idx: m.index, src });
    }
  }
  if (!entries.length) return null;

  // Prefer a year near a JanΓÇôDec span (e.g., "...January_December_2024...")
...
function classifyPeriod({ text = "", href = "", neighbor = "", year }) {
  ...
  // 3) If month words AND only ONE distinct year near the chosen year ΓåÆ monthly
  if (MONTH_RE.test(s) && years.size <= 1 && typeof year === "number") {
    ...
    if (idx !== -1) {
      ...
      if (MONTH_RE.test(seg) && !/december|α╕ÿα╕▒α╕Öα╕ºα╕▓α╕äα╕í/.test(seg)) {
        return "monthly";
      }
    }
  }
  ...
  if (/energy[_\s-]?balance|energy[_\s-]?commodity[_\s-]?account|α╕öα╕╕α╕Ñα╕óα╕áα╕▓α╕₧α╕₧α╕Ñα╕▒α╕çα╕çα╕▓α╕Ö/i.test(s)) return "annual";

  // 5) No month words ΓåÆ annual-ish
...
async function downloadIfNew(item, product) {
  ...
  if (fs.existsSync(outPath) && !CONFIG.forceDownload) {
    if (CONFIG.DEBUG) console.log(`Γ£ô file exists: ${outPath}`);
  ...
  if (!CONFIG.forceDownload && already && fs.existsSync(already)) {
    if (CONFIG.DEBUG) console.log(`Γ£ô already by URL: ${item.text || item.href}`);
    return;
  }

  console.log(`Γåô downloading: ${item.text || originalName}`);
